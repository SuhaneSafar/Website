      ...
      <div className="relative w-full flex flex-col">
        {teamData.length > 0 ? (
          <>
            {/* Core Members Slider */}
            {(() => {
              const coreMembers = teamData.filter(m => !m.dept || m.dept.toLowerCase() === 'core');
              if (coreMembers.length === 0) return null;
              return (
            ...
            {/* Department Members Slider */}
            {(() => {
              const nonCore = teamData.filter(m => m.dept && m.dept.toLowerCase() !== 'core');
              if (nonCore.length === 0) return null;
              
              const deptGroups = {};
              nonCore.forEach(member => {
              ...
              Object.keys(deptGroups).forEach(deptName => {
                deptSlides.push({ deptName, data: deptGroups[deptName] });
              });
                ...
                <div className="relative w-full -mt-4">
                  <div className="w-full flex items-center justify-center mb-6">
                    <h2 className="text-3xl font-extrabold text-slate-800 relative inline-block">
                      Our Teams
                      <span className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-16 h-1 bg-sky-500 rounded-full"></span>
                    </h2>
                  </div>
                  <Swiper
                    modules={[Navigation, Pagination, Autoplay]}
                    ...
                    className="pb-16"
                  >
                    {deptSlides.map((slide, slideIdx) => (
                      <SwiperSlide key={`dept-${slideIdx}`} className="h-auto flex justify-center pb-4 pt-2">
                        <div className="relative bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-4 sm:p-5 flex flex-col w-full h-full max-w-[420px] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-transparent hover:border-sky-100">
                          <div className="w-full text-center border-b border-sky-100/50 pb-3 mb-4 shrink-0">
                            <h3 className="text-lg sm:text-xl font-extrabold text-slate-800 uppercase tracking-widest">{slide.deptName}</h3>
                          </div>
                          <div className="grid grid-cols-3 gap-x-2 sm:gap-x-3 gap-y-2 sm:gap-y-3 w-full flex-grow content-start">
                            {slide.data.map((member, idx) => (
                              <div key={member.id} className={`relative group flex flex-col items-center text-center p-1 sm:p-2 rounded-xl hover:bg-slate-50 transition-colors ${slide.data.length % 3 === 1 && idx === slide.data.length - 1 ? 'col-start-2' : ''}`}>
                                {isAdmin && (
                                ...
                                <p className="text-[10px] sm:text-[11px] text-slate-500 mt-2 px-1 text-center leading-relaxed">{member.desc}</p>
                              </div>
                            ))}
                          </div>
                        </div>
