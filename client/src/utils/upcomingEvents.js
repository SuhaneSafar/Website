const upcomingEvents = [
  {
    id: 1,
    title: "WAYANAD",
    subtitle:
      "Join the exciting community trips to Wayanad with amazing discounts",
    mainImage: "/Images/Wayanad-main.png",
    gallery: [
      "/Images/wayanad1.png",
      "/Images/wayanad2.png",
      "/Images/wayanad3.png",
    ],
    reviews: [
      { name: "Amit", rating: 5, text: "Amazing trip, well organized!" },
      { name: "Priya", rating: 4, text: "Loved the food and the group!" },
    ],
    pickupDrop: "Bangalore - Wayanad - Bangalore",
    duration: "3 Days / 2 Nights",
    inclusions: [
      "Accommodation 2 night stay in resort in Wyanad",
      "5 Meals ( 3 Breakfadt + 2 Dinner )",
      "Transportation Banglore to Banglore by traveller/cab",
      "Surprise games and gifts",
      "Swimming pool",
      "All kninds of permits",
      "First aid kits",
      "Driver allowance, toll tax and parking",
    ],
    exclusions: [
      "Any personal expenses (laundary, shopping, tips, etc.)",
      "Any adveture activities not mentioned",
      "Entrance fees of any monuments",
      "Travel insurance",
    ],
    overview:
      "Experience the thrill of Manali with our community trips. Enjoy scenic views, group activities, and exclusive discounts. Perfect for adventure seekers and nature lovers!",
    itinerary: [
      {
        day: 0,
        title: "14th August(Nigh Departure)",
        description: "Departure from Banglore at 10:00 PM. Overnight journey to Wayanad." 
      },
      {
        day: 1,
        title: "15th August(Nature and Heritage)",
        description:
          "Arrival and freshen up at resort, breakfast. Visit Eddakal Lake(boating and forest walk). Quick stop at Lakkidi Viewpoint. Bonfire, group dinner and interaction. Overnight stay at resort.",
      },
      {
        day: 2,
        title: "16th August(Waterfalls & Culture)",
        description:
          "Breakfast at resort. Visit Banasura Sagar Dam. Meenmutty Waterfalls(viewpoint only). Lunch on own. Explore Uravu Bamboo Craft Village. Cultural night, group dinner, fun activities. Overnight stay at resort.",
      },
      {
        day: 7,
        title: "17th August(Leisure and Return)",
        description:
          "Breakfast and checkout. Visit Karlad Lake(optional kayaking). Local shopoing(tea, spices, souvenirs). Lunch on own. Departure for Banglore at 1:30 PM. Arrival in Banglore by 9:00-10:00 PM",
      },
    ],
    //buttonLabel: "Know more",
  },
  {
    id: 2,
    title: "CHANDRSHILLA",
    subtitle: "Join the exciting community trips with amazing discounts",
    mainImage: "/Images/chandrshilla.jpg",
    gallery: [
      "/Images/chandrashila1.jpg",
      "/Images/chandrashila2.png",
      "/Images/chandrashila3.jpg",
    ],
    reviews: [
      {
        name: "Mohit",
        rating: 4,
        text: "The views were breathtaking.",
      },
      {
        name: "Snehal",
        rating: 5,
        text: "The local guide was very knowledgeable and friendly.",
      },
    ],
    pickupDrop: "IIT Delhi | IIT Roorkee | Deheradun | Chandigarh | Chopta",
    duration: "3 Days / 2 Nights",
    inclusions: [
      "Round trip transportation from Delhi/Roorkee/Chandigarh/Deheradun to Chopta",
       "2 Nights homestay/camp stay at Chopta or Makkumath(shared rooms)",
       "5 Meals:2 breakfast, 2 dinners, 1 luncg/snacks",
       "Local exploration: Omakreshwar Temple, Chopta meadows, Ukhimath", 
       "Guided trek to Tungnath and Chandrashila",
       "All required permits",
       "Surprise group games and gifts",
       "Basic first aid kit",
       "Toll tax, parking, driver allowance included"
       ],
    exclusions: [
      "Personal expenses(shopping, snacks, laundry, tips)",
       "Entry fees to temples or parks(if applicable)",
        "Adventure activities not mentioned", 
        "Travel insurance", 
        "Anything not specifically listed in the inclusions"],
    overview:
      "Discover the untouched beauty of Chandrashila. Our trips offer a blend of adventure, relaxation, and cultural experiences. Book now for special offers!",
    itinerary: [
      {
        day: 1,
        title: "Departure from  IIT Delhi(1:00 AM), Roorkee(4:00 AM), Deheradun (5:00 AM) or Chandigarh (1:00 AM)",
        description: "Scenic drive through GArwal hills. Visit Omkareshwar Temple Temple at Ukhimath. Transfer to Chopta/Makkumath. Dinner and interactive fun night.",
      },
      {
        day: 2,
        title: "Early morning drive to Chopta.",
        description: "Trek to Tungnath Temple(6 km) and Chandrashila Peak(2km). Return to Chopta by evening Dinner and star - gazing night stay.",
      },
      {
        day: 3,
        title: "Breakfast and group photo.",
        description: "Departure for respective cities. Reach back by evening with unforgettable memories and friends.",
      },
    ],
   // buttonLabel: "Know more",
  },
  {
    id: 2,
    title: "UDAIPUR",
    subtitle: "Join the exciting community trips with amazing discounts",
    mainImage: "/Images/Udaipur-main.png",
    gallery: [
      "/Images/udaipur1.jpg",
      "/Images/udaipur2.jpg",
      "/Images/udaipur3.jpg",
    ],
    reviews: [
      {
        name: "Rahul",
        rating: 5,
        text: "Absolutely fantastic! The views were breathtaking.",
      },
      {
        name: "Sneha",
        rating: 4,
        text: "The local guide was very knowledgeable and friendly.",
      },
    ],
    pickupDrop: "Delhi NCR",
    duration: "3 Days / 2 Nights",
    inclusions: [
      "All travel by cab/traveller (Delhi NCR - Udaipur - Kumbhalgarh - return",
       "2 Night hotel Stay in Udaipur", "2 Breakfasts and 2 Dinners", "Sightseeing and entry coordination as per itinerary",
       "Suhane Safar trip leader and local guide","Surprise games and bonding activities", "Swimming pool access at stay",
       "Permits, tolls, parking and driver allowance", " First aid support"
       ],
    exclusions: ["Personal expenses(shopping, laundry, tips, etc.)", "Entry fees for monuments, boat rides or folks shows", "Travel insurance", "Early check-in stay(subject to availability)", "Anything not mentioned under inclusions"],
    overview:
      "Discover the untouched beauty of Harshil Valley. Our trips offer a blend of adventure, relaxation, and cultural experiences. Book now for special offers!",
    itinerary: [
      {
        day: 0,
        title: "Departure from Delhi NCR at 8 PM",
        description: "Overnight journey to Udaipur.",
      },
      {
        day: 1,
        title: "Sightseeing:City Palace,Lake Pichola, Jagdish Temple, Ghangaur Ghat",
        description: "Evening: Optional folk performance at Bagore Ki Haveli or local flea market visit. Dinner and overnight stay at hotel with fun activities.",
      },
      {
        day: 2,
        title: "Visit Saheliyon ki Bari and Fateh Sagar Lake",
        description: "Optional boat ride available (personal cost). Dinner, games and overnight hotel stay.",
      },
      {
        day: 3,
        title: "Checkout and travel to kumbhalgarh Fort",
        description: "Explore the UNESCO World Heritage Site. Evening departure to Delhi NCR. Overnight journey.",
      },
      {
        day: 4,
        title: "Early morning arrival",
        description: " Delhi/Gurugram",
      }
    ],
   // buttonLabel: "Know more",
  },
  // {
  //   id: 3,
  //   title: "Harshil Valley",
  //   subtitle: "Join the exciting community trips with amazing discounts",
  //   mainImage: "/Images/harshil-main.png",
  //   gallery: [
  //     "/Images/harshil-1.jpg",
  //     "/Images/harshil-2.jpg",
  //     "/Images/harshil-3.jpg"
  //   ],
  //   reviews: [
  //     { name: "Rahul", rating: 5, text: "Absolutely fantastic! The views were breathtaking." },
  //     { name: "Sneha", rating: 4, text: "The local guide was very knowledgeable and friendly." }
  //   ],
  //   pickupDrop: "Delhi - Harshil - Delhi",
  //   duration: "3 Days / 2 Nights",
  //   inclusions: [
  //     "Morning and night meals",
  //     "Stay",
  //     "Transport"
  //   ],
  //   exclusions: [
  //     "Lunch",
  //     "Activities"
  //   ],
  //   overview: "Discover the untouched beauty of Harshil Valley. Our trips offer a blend of adventure, relaxation, and cultural experiences. Book now for special offers!",
  //   itinerary: [
  //     { day: 1, title: "Departure from Delhi", description: "Board the bus from Delhi in the evening." },
  //     { day: 2, title: "Arrival in Harshil", description: "Check-in, local sightseeing, group activities." },
  //     { day: 3, title: "Departure from Harshil", description: "Board the bus back to Delhi." }
  //   ],
  //   buttonLabel: "Know more"
  // }
];

export default upcomingEvents;
